  ...
  const handleDeleteCategory = (categoryId: string) => {
    if (window.confirm('Are you sure you want to delete this category and all its items?')) {
      if (isDemoMode()) {
        ...
        const itemsToDelete = menuItems.filter(item => item.category_id === categoryId);
        itemsToDelete.forEach(item => {
          const allItems = DemoStorage.getFromStorage<any>('menufy_items');
          const filtered = allItems.filter((i: any) => i.id !== item.id);
          DemoStorage.saveToStorage('menufy_items', filtered);
        });

        // Delete category
        const allCategories = DemoStorage.getFromStorage<any>('menufy_categories');
        const filtered = allCategories.filter((c: any) => c.id !== categoryId);
        DemoStorage.saveToStorage('menufy_categories', filtered);
